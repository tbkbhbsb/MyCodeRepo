...
def xy_list_categorise_length_plot(XList, YList, CategoriseList, WorkNameList, PlotFilePath, x_axis_name, y_axis_name):
    ...
    Data = pd.DataFrame({'plots': XList, 'characters': YList,
                         'length_of_plot': CategoriseList, 'work_name': WorkNameList})
    print(Data)
...
def xy_list_categorise_works_plot(XList, YList, CategoriseList, WorkNameList,
                                  PlotFilePath, x_axis_name, y_axis_name):
    ...
    Data = pd.DataFrame({
        'plots': XList,
        'characters': YList,
        'length_of_plot': CategoriseList,
        'work_name': WorkNameList})
    print(Data)
    Data.to_csv(data_export_path, index=False)
    ...
    plot_object + geom_line(alpha="0.5") + \
        xlab(x_axis_name) + ylab(y_axis_name) + \
        theme_gray(base_family = "HiraKakuProN-W3")
    # theme_gray(base_family = "Japan1")+

    ggsave(file=PlotFilePath, plot=plot_object,
           units="cm", width=32, height=18, device="cairo_ps")
